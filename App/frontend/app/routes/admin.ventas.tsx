...
import { CheckCircle2, Package, Plus, ShoppingCart, Trash2, XCircle } from 'lucide-react';
...
import { errMsg, formatCordobas } from '~/lib/utils';
import {
...
export default function AdminVentas() {
        ...
        row.original.status === 'pending_approval' ? (
          <div className="flex justify-end gap-1.5">
            <Button size="sm" variant="ghost" onClick={() => handleApprove(row.original.id)}>
              <CheckCircle2 className="h-4 w-4 text-accent-2" aria-hidden />
            </Button>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-extrabold tracking-tight text-text">Ventas</h2>
        <p className="text-muted">Cotizá, registrá{isAdmin ? ' y aprobá' : ''} tus ventas.</p>
      </div>
      ...
      <Tabs defaultValue="quote" className="space-y-6">
        ...
        <TabsContent value="quote" className="space-y-6">
          <Card className="bg-surface border-border shadow-sm">
            <CardHeader>
              <CardTitle className="text-lg text-text">Agregar producto</CardTitle>
          ...
          {lines.length > 0 && (
            <Card className="bg-surface border-border shadow-sm">
              <CardHeader>
                <CardTitle className="text-lg text-text">Líneas de la venta</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                  ...
                  {lines.map((line) => (
                    <div key={line.productName} className="flex items-center justify-between gap-3 px-3 py-2">
                      <div className="min-w-0 flex-1">
                        <p className="truncate font-medium text-text">{line.productName}</p>
                ...
                <div className="flex items-center justify-between border-t border-border pt-3">
                  ...
                  <span className="text-xl font-bold text-accent-2 tabular-nums">{formatCordobas(total)}</span>
                </div>
                ...
                <div className="grid gap-3 sm:grid-cols-[1fr_auto_auto] sm:items-end">
                  ...
                  <Button variant="outline" onClick={handleQuote} loading={quoting}>
                    Cotizar
                  </Button>
          ...
          {quote && (
            <Card className="bg-surface border-border shadow-sm">
              <CardHeader>
                <CardTitle className="text-lg text-text">Cotización</CardTitle>
                <CardDescription className="text-muted">
              ...
              <CardContent className="space-y-3">
                {quote.lines.map((line) => (
                  <div key={line.productName} className="rounded-lg border border-border p-3 text-sm">
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-text">{line.productName}</span>
                      <span className="font-semibold text-text">{formatCordobas(line.precioUnit)} c/u</span>
                    </div>
                    <div className="mt-2 grid grid-cols-2 gap-2 text-xs text-muted sm:grid-cols-4">
                      <span>
                        Comisión: <span className="font-medium text-accent-2">{formatCordobas(line.comision)}</span> (
                        {Math.round(line.comisionPercent * 100)}%)
                      </span>
                      <span>
                        Ganancia tienda: <span className="font-medium text-accent-2">{formatCordobas(line.gananciaTienda)}</span>
                      </span>
                      {line.wholesale.discountPercent > 0 && (
                        <span>Mayoreo: −{Math.round(line.wholesale.discountPercent * 100)}%</span>
                      )}
                      {line.insufficientStock && (
                        <span className="font-medium text-danger">Stock insuficiente (disp. {line.available})</span>
                      )}
                    </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 border-t border-border pt-3 text-sm">
                  <span className="text-muted">
                    Comisión total: <span className="font-semibold text-text">{formatCordobas(quote.totalComision)}</span>
                  </span>
                  <span className="text-muted">
                    Ganancia tienda: <span className="font-semibold text-text">{formatCordobas(quote.totalGananciaTienda)}</span>
                  </span>
                  <span className="text-lg font-bold text-accent-2">{formatCordobas(quote.total)}</span>
                </div>
              </CardContent>
            </Card>
          )}
          ...
          {lines.length === 0 && !quote && (
            <div className="rounded-lg border border-dashed border-border bg-surface-2/50 py-12 text-center">
              <Package className="mx-auto mb-3 h-10 w-10 text-muted opacity-50" aria-hidden />
              <p className="font-medium text-text">Todavía no agregaste productos.</p>
              <p className="mt-1 text-sm text-muted">Usá el formulario de arriba para armar la venta.</p>
            </div>
          )}
        </TabsContent>
      ...
      <Dialog open={!!rejectFor} onOpenChange={(open) => !open && setRejectFor(null)}>
        <DialogContent>
          ...
          <div className="mt-4 flex justify-end gap-2">
            ...
            <Button onClick={handleReject} loading={rejecting} className="bg-danger/90">
              Rechazar
            </Button>
          </div>
        </DialogContent>
